...
<head>
    ...
    <style>
        ...
        .dashboard-overlay {
            background: rgba(0, 0, 0, 0.5); /* Un poco más oscuro para que se lea el texto */
            position: absolute;
...
<body>
    <div class="wrapper">
                    ...
                    <div class="col-12">
                        <div class="dashboard-header">
                            <div class="dashboard-overlay">
                                <h1 class="h3 text-white font-weight-bold mb-1">Bienvenido</h1>
                                <h2 class="h5 text-white-op">
                                    <?= esc($nombre) ?> <?= esc($apellidos) ?>
                                </h2>
                                <small class="text-white-50">Ciclo Escolar 2025-2026</small>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-header bg-dark text-white">
                    <h5 class="modal-title" style="font-size: 1rem;">Cambiar Contraseña</h5>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Instancia del modal de Bootstrap (para poder cerrarlo por código)
            const myModal = new bootstrap.Modal(document.getElementById('modalPassword'));

            // 1. REGLA: PROHIBIDO ESPACIOS (EN TIEMPO REAL)
            inputPass.addEventListener('input', function() {
                // Si el usuario escribe un espacio, lo borramos inmediatamente
            ...
            form.addEventListener('submit', function(e) {
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        ...
                        setTimeout(() => {
                            // Ocultar modal usando la instancia de Bootstrap 5
                            // Nota: Si usas Bootstrap 4, sería $('#modalPassword').modal('hide');
                            // Como tu plantilla parece moderna, usaremos el método nativo:
                            const modalEl = document.getElementById('modalPassword');
                            const modalInstance = bootstrap.Modal.getInstance(modalEl) || new bootstrap.Modal(modalEl);
                            modalInstance.hide();

                            // Restaurar botón por si lo vuelven a abrir sin recargar
                            btn.disabled = false;
